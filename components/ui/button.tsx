  ...
  "inline-flex items-center justify-center whitespace-nowrap rounded-md text-sm font-medium ring-offset-background transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50 [&_svg]:pointer-events-none [&_svg]:size-4 [&_svg]:shrink-0",
  {
    variants: {
      variant: {
        ...
        calm: "bg-light-mint text-teal-800 hover:bg-light-mint/90 border border-teal/20",
      },
      size: {
...
const Button = React.forwardRef<HTMLButtonElement, ButtonProps>(
  ({ className, variant, size, asChild = false, ...props }, ref) => {
    ...
    // Add ARIA attributes for gradient buttons
    const ariaProps: Record<string, string> = {}
    if (className?.includes("gradient")) {
      ariaProps["aria-label"] = props.children?.toString() || props["aria-label"] || "Button"
    }

    // Add slower transitions for users who prefer reduced motion
    const transitionClass = prefersReducedMotion ? "transition-none" : "transition-all duration-300"

    const Comp = asChild ? Slot : "button"
    return (
      <Comp
        className={cn(buttonVariants({ variant, size, className }), transitionClass)}
        ref={ref}
        {...ariaProps}
        {...props}
      />
