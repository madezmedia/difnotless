...
// Using a non-JSX approach for .ts file
import { createElement } from "react"

export const PortableText = (props: { value: PortableTextBlock[] }) => {
  return createElement(PortableTextComponent, {
    value: props.value,
    components: {
      types: {
        image: ({ value }: { value: any }) => {
          ...
          const imgUrl = urlForImage(value).width(800).height(450).fit("max").auto("format").url() || "/placeholder.svg";
          
          return createElement("div", { className: "my-6 space-y-2" }, [
            createElement("img", {
              key: "img",
      ...
      marks: {
        link: ({ children, value }: any) => {
          // Only proceed if we have a valid value with href
